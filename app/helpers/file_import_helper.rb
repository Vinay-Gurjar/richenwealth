...
module FileImportHelper
  ...
  def create_reports(row)
    not_create_entry = ''
    user = user_by_email(row[2])
    if user.present?
      if user.has_role?(:agent)
        hr = HourlyReport.new
        hr.call_center_id = current_user&.call_center&.id
        hr.user_id = user.id
        hr.login_time = row[5].present?  ? DateTime.parse(row[5]) : ''
        hr.logout_time = row[6].present?  ? DateTime.parse(row[6]) : ''
        ...
        hr.report_time = ShiftWiseTime.find_by(id: params[:rt_id])
        hr.date = Date.parse(params[:date])
        hr.save!
      else
        not_create_entry =  { user_name: row[1], reason: 'This is not caller in our database' }
      end
    else
